...
def fetch_data(conn, query):
    ...
    print(f"Se han encontrado {len(data)} registros.")
    print("----------------- FILAS -----------------")
    for row in data:
        print(row)
    print("----------------- FILAS -----------------")
...
def sync_tables(postgres_conn, oracle_conn):
    postgres_query = "SELECT * FROM usuarios"
    oracle_query = "SELECT * FROM usuarios"
    print("\n---------------- *[INCIO POSTGRES]*-----------------\n")
    ...
    if keys_to_insert:
        ...
        insert_query = f"INSERT INTO usuarios ({', '.join(pg_columns)}) VALUES ({placeholders})"
    ...
    if keys_to_update:
        ...
        update_query = f"UPDATE usuarios SET {set_clause} WHERE {pg_columns[0]} = :{len(update_columns)+1}"
    ...
    if keys_to_delete:
        delete_query = f"DELETE FROM usuarios WHERE {pg_columns[0]} = :1"
